...
/*
	mutations:{
		...
		setpresaleList (state, data){
			state.presaleList = data;
		},
		setpresaledetailsList (state, data){
			state.presaledetailsList = data;
		},
		setfunctionList(state, data){
			state.functionList = data;
		},
		setimplementationList(state, data){
			state.implementationList = data;
		},
		setresearchList(state, data){
	...
	actions:{
		...
		getpresaleList(context){
			axios.get('http://localhost:2411/api/presaleList')
				.then(function(res){
					var a = JSON.parse(res.data)
					setTimeout(() => {
						context.commit('setpresaleList', a)
					}, 0);
				})
				.catch(function(error){
					console.log(error);
		...
		getpresaledetailsList(context){
			axios.get('http://localhost:2411/api/presaledetailsList')
				.then(function(res){
					var a = JSON.parse(res.data)
					setTimeout(() => {
						context.commit('setpresaledetailsList', a)
					}, 0);
				})
				.catch(function(error){
					console.log(error);
		...
		getimplementationList(context){
			setTimeout(() => {
				context.commit('setimplementationList', implementationlist)
			}, 0);
		},
		getresearchList(context){
			setTimeout(() => {
				context.commit('setresearchList', researchlist)
			}, 0);
		},
		getprojectList(context){
			axios.get('http://localhost:2411/api/projectinfo')
				.then(function(res){
					var a = JSON.parse(res.data)
					setTimeout(() => {
						context.commit('setprojectList', a)
					}, 0);
				})
				.catch(function(error){
					console.log(error);
		...
		getlogin(context){
			axios.get('http://localhost:2411/api/login')
				.then(function(res){
					var a = JSON.parse(res.data)
					setTimeout(() => {
						context.commit('setlogin', a)
					}, 0);
				})
				.catch(function(error){
					console.log(error);
